class parser:
    ...
    def get(self,BOOLSET=False):
        if BOOLSET:
            return self.ITEMS, self.STRUCTURS, self.CRAFTS, self.sITEMS, self.sSTRUCTURS
        return self.ITEMS, self.STRUCTURS, self.CRAFTS
    ...
    def _ParserCraft(self, DownloadVar):
        def IDITEMSF(text):
            if self.sITEMS.get(text) is not None:
                return self.sITEMS[text]
            else:
                return -1
        def RETURNINT(text):
